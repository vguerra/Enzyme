  ...
  if (isa<Argument>(oval) && cast<Argument>(oval)->hasByValAttr()) {
    ...
    if (!hasMetadata(arg, "enzyme_shadow")) {
      ...
      if ((mode == DerivativeMode::ReverseModeCombined ||
        ...
        endCheck:;
          if (!seen) {
            ...
            if (width > 1) {
              return applyChainRule(arg->getType(), antialloca, bb, rule2);
            } else {
      ...
      if (arg->hasInternalLinkage() || arg->hasPrivateLinkage() ||
        ...
        Value *shadow;
        if (width > 1) {
          shadow = applyChainRule(oval->getType(), ip, BuilderM, rule);
        } else {
          shadow = rule(ip);
        }
    ...
    Value *shadow;
    if (width > 1) {
      shadow = applyChainRule(shadowTy, invertOp, bb, rule);
    } else {
    ...
    if (arg->isCast()) {
      ...
      if (auto C = dyn_cast<Constant>(ip)) {
        ...
        if (width > 1) {
          return applyChainRule(arg->getType(), ip, bb, rule);
        } else {
          return rule(ip);
        }
        ...
        Value *shadow;
        if (width > 1) {
          shadow = applyChainRule(arg->getType(), ip, bb, rule);
        } else {
          shadow = rule(ip);
        }
    ...
    Value *shadow;
    if (width > 1) {
      shadow = applyChainRule(arg->getType(), ip0, ip1, bb, rule);
    } else {
      shadow = rule(ip0, ip1);
    }
    ...
    Value *li;
    if (width > 1) {
      li = applyChainRule(arg->getType(), ip, bb, rule);
    } else {
      li = rule(ip);
    }
    ...
    Value *li;
    if (width > 1) {
      li = applyChainRule(arg->getType(), val0, val1, bb, rule);
    } else {
      li = rule(val0, val1);
    }
    ...
    if (auto ci = dyn_cast<ConstantInt>(asize)) {
      if (ci->isOne()) {
        ...
        if (width > 1) {
          applyChainRule(antialloca, bb, rule);
        } else {
          rule(antialloca);
        }
        return antialloca;
    ...
    if (width > 1) {
      applyChainRule(antialloca, bb, rule2);
    } else {
      rule2(antialloca);
    }
