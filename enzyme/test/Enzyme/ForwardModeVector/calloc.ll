...
entry:
  ...
  %call = call %struct.Gradients (i8*, ...) @__enzyme_fwdvectordiff(i8* bitcast (double (double,i64)* @f to i8*), i32 %0, double %1, %struct.Gradients { double 1.0, double 2.0, double 3.0 }, i64 1)
  ret %struct.Gradients %call
}

declare dso_local %struct.Gradients @__enzyme_fwdvectordiff(i8*, ...)


; CHECK: efine internal [3 x double] @fwdvectordiffef(double %x, [3 x double] %"x'", i64 %arg)
; CHECK-NEXT: entry:
