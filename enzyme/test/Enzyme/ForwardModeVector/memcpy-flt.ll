...
; Function Attrs: nounwind uwtable
define dso_local void @dmemcpy_float(double* %dst, %struct.Gradients %dstp, double* %src, %struct.Gradients %srcp, i64 %n) local_unnamed_addr #0 {
entry:
  tail call void (...) @__enzyme_fwdvectordiff.f64(void (double*, double*, i64)* nonnull @memcpy_float, double* %dst, %struct.Gradients %dstp, double* %src, %struct.Gradients %srcp, i64 %n) #3
  ret void
}
...
; CHECK: define internal void @fwdvectordiffememcpy_float(double* nocapture %dst, [3 x double*] %"dst'", double* nocapture readonly %src, [3 x double*] %"src'", i64 %num)
; CHECK-NEXT: entry:
