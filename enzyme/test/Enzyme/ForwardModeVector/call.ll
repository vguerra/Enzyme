...
; CHECK: define internal [3 x double] @fwdvectordiffe_Z4add4d(double %x, [3 x double] %"x'")
; CHECK-NEXT: entry:
; CHECK-NEXT:   %0 = call {{(fast )?}}[3 x double] @fwdvectordiffe_Z4add2d(double %x, [3 x double] %"x'")
; CHECK-NEXT:   ret [3 x double] %0
; CHECK-NEXT: }

; CHECK: define internal [3 x double] @fwdvectordiffe_Z4add2d(double %x, [3 x double] %"x'") {
; CHECK-NEXT: entry:
