...
%struct.Gradients = type { i8*, i8*, i8* }

define void @caller(i8* %a, %struct.Gradients %b, i8* %c) local_unnamed_addr {
entry:
  call void @__enzyme_fwdvectordiff(i8* bitcast (void (double**, i64*)* @_ZL6matvecPKN5Eigen6MatrixIdLin1ELin1ELi0ELin1ELin1EEES3_ to i8*), i8* %a, %struct.Gradients %b, i8* %c)
  ret void
}

declare void @__enzyme_fwdvectordiff(i8*, i8*, %struct.Gradients, i8*) local_unnamed_addr
...
; CHECK: define internal void @fwdvectordiffesubcall(double** %m_data.i.i.i, [3 x double**] %"m_data.i.i.i'", i64* %tmp7)
; CHECK-NEXT: entry:
...
; CHECK: or.body:                                         ; preds = %for.body, %entry
...
; CHECK-NEXT:   %9 = insertelement <3 x double> undef, double %3, i64 0
; CHECK-NEXT:   %10 = insertelement <3 x double> %9, double %4, i64 1
; CHECK-NEXT:   %11 = insertelement <3 x double> %10, double %5, i64 2
...
; CHECK-NEXT:   %12 = fmul fast <3 x double> %8, %.splat
; CHECK-NEXT:   %.splatinsert5 = insertelement <3 x double> {{poison|undef}}, double %ld, i32 0
; CHECK-NEXT:   %.splat6 = shufflevector <3 x double> %.splatinsert5, <3 x double> {{poison|undef}}, <3 x i32> zeroinitializer
; CHECK-NEXT:   %13 = fmul fast <3 x double> %11, %.splat6
...
; CHECK-NEXT:   store double %15, double* %"call'ipg", align 8
; CHECK-NEXT:   store double %16, double* %"call'ipg3", align 8
; CHECK-NEXT:   store double %17, double* %"call'ipg4", align 8
