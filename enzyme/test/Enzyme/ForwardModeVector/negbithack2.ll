...
define %struct.Gradients @test_derivative(<2 x double> %x, %struct.Gradients %dx) {
entry:
  %0 = tail call %struct.Gradients (<2 x double> (<2 x double>)*, ...) @__enzyme_fwdvectordiff(<2 x double> (<2 x double>)* nonnull @tester, <2 x double> %x, %struct.Gradients %dx)
  ret %struct.Gradients %0
}

; Function Attrs: nounwind
declare %struct.Gradients @__enzyme_fwdvectordiff(<2 x double> (<2 x double>)*, ...)


; CHECK: define internal [3 x <2 x double>] @fwdvectordiffetester(<2 x double> %x, [3 x <2 x double>] %"x'")
; CHECK-NEXT: entry:
