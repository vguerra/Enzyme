...
; extern array<array<double,3>,3> __enzyme_fwdvectordiff(void*, ...);

; array<double,3> square(double x) {
;     return {x * x, x * x * x, x};
; }
; array<array<double,3>,3> dsquare(double x) {
;     // This returns the derivative of square or 2 * x
;     return __enzyme_fwdvectordiff((void*)square, x, (array<double,3>){1.0, 2.0, 3.0});
; }
; int main() {
;     printf("%f \n", dsquare(3)[0][0]);
; }
...
define dso_local void @_Z7dsquared(%struct.OutGradients* noalias sret(%struct.OutGradients) align 8 %agg.result, double %x) local_unnamed_addr #1 {
entry:
  call void (%struct.OutGradients*, i8*, ...) @_Z22__enzyme_fwdvectordiffPvz(%struct.OutGradients* sret(%struct.OutGradients) align 8 %agg.result, i8* bitcast (void (%"struct.std::array"*, double)* @_Z6squared to i8*), double %x, %struct.InGradients* nonnull byval(%struct.InGradients) align 8 @__const._Z7dsquared.dx)
  ret void
}

declare void @llvm.lifetime.start.p0i8(i64 immarg, i8* nocapture) #2

declare dso_local void @_Z22__enzyme_fwdvectordiffPvz(%struct.OutGradients* sret(%struct.OutGradients) align 8, i8*, ...) local_unnamed_addr #3
...
entry:
  ...
  call void @_Z7dsquared(%struct.OutGradients* nonnull sret(%struct.OutGradients) align 8 %ref.tmp, double 3.000000e+00)
