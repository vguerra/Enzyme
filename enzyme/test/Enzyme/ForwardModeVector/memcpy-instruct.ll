...
; Function Attrs: nounwind uwtable
define dso_local void @dmemcpy_ptr(i8* %dst, %struct.Gradients %dstp, i8* %src, %struct.Gradients %srcp, i64 %n) {
entry:
  tail call void (...) @__enzyme_fwdvectordiff.f64(void (i8*, i8*, i64)* nonnull @memcpy_ptr, metadata !"enzyme_dup", i8* %dst, %struct.Gradients %dstp, metadata !"enzyme_dup", i8* %src, %struct.Gradients %srcp, i64 %n)
  ret void
}
...
; CHECK: define internal void @fwdvectordiffememcpy_ptr(i8* nocapture %dst, [3 x i8*] %"dst'", i8* nocapture readonly %src, [3 x i8*] %"src'", i64 %num)
; CHECK-NEXT: entry:
