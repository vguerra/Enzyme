...
entry:
  %res = tail call %struct.Gradients (...) @__enzyme_fwdvectordiff.f64(void (<2 x double>*, <2 x i1>, <2 x double>)* @loader, i8* %ptr, %struct.Pointers %dptr, <2 x i1> %mask, <2 x double> %other, %struct.Gradients %dother)
  ret %struct.Gradients %res
}


; CHECK: define internal void @fwdvectordiffeloader(<2 x double>* %ptr, [3 x <2 x double>*] %"ptr'", <2 x i1> %mask, <2 x double> %val, [3 x <2 x double>] %"val'")
; CHECK-NEXT: entry:
