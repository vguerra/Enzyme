...
define void @main(double* %a, %struct.InGradients %da, i32* %N) {
entry:
  %call = call %struct.OutGradients (i8*, ...) @__enzyme_fwdvectordiff(i8* bitcast (double (double*, i32*)* @alldiv to i8*), double* nonnull %a, %struct.InGradients %da, i32* nonnull %N)
  ret void
}
...
; CHECK: define internal [3 x double] @fwdvectordiffealldiv(double* noalias nocapture %a, [3 x double*] %"a'", i32* noalias nocapture %N)
; CHECK-NEXT: entry:
...
; CHECK-NEXT:   %"sum.134'" = phi {{(fast )?}}[3 x double] [ %"sum.036'", %for.body4.lr.ph ], [ %35, %for.body4 ]
...
; CHECK-NEXT:   %"arrayidx'ipg3" = getelementptr inbounds double, double* %6, i64 %4
; CHECK-NEXT:   %7 = extractvalue [3 x double*] %"a'", 2
; CHECK-NEXT:   %"arrayidx'ipg4" = getelementptr inbounds double, double* %7, i64 %4
...
; CHECK-NEXT:   %10 = load double, double* %"arrayidx'ipg3", align 8
; CHECK-NEXT:   %11 = load double, double* %"arrayidx'ipg4", align 8
; CHECK-NEXT:   %12 = insertelement <3 x double> undef, double %9, i64 0
; CHECK-NEXT:   %13 = insertelement <3 x double> %12, double %10, i64 1
; CHECK-NEXT:   %14 = insertelement <3 x double> %13, double %11, i64 2
; CHECK-NEXT:   %.splatinsert = insertelement <3 x double> {{poison|undef}}, double %8, i32 0
; CHECK-NEXT:   %.splat = shufflevector <3 x double> %.splatinsert, <3 x double> {{poison|undef}}, <3 x i32> zeroinitializer
; CHECK-NEXT:   %15 = fmul fast <3 x double> %14, %.splat
; CHECK-NEXT:   %16 = fadd fast <3 x double> %15, %15
; CHECK-NEXT:   %17 = extractelement <3 x double> %16, i64 0
; CHECK-NEXT:   %18 = extractelement <3 x double> %16, i64 1
; CHECK-NEXT:   %19 = extractelement <3 x double> %16, i64 2
; CHECK-NEXT:   %20 = extractvalue [3 x double] %"sum.134'", 0
; CHECK-NEXT:   %21 = insertelement <3 x double> undef, double %20, i64 0
; CHECK-NEXT:   %22 = extractvalue [3 x double] %"sum.134'", 1
; CHECK-NEXT:   %23 = insertelement <3 x double> %21, double %22, i64 1
; CHECK-NEXT:   %24 = extractvalue [3 x double] %"sum.134'", 2
; CHECK-NEXT:   %25 = insertelement <3 x double> %23, double %24, i64 2
; CHECK-NEXT:   %26 = insertelement <3 x double> undef, double %17, i64 0
; CHECK-NEXT:   %27 = insertelement <3 x double> %26, double %18, i64 1
; CHECK-NEXT:   %28 = insertelement <3 x double> %27, double %19, i64 2
; CHECK-NEXT:   %29 = fadd fast <3 x double> %25, %28
; CHECK-NEXT:   %30 = extractelement <3 x double> %29, i64 0
; CHECK-NEXT:   %31 = insertvalue [3 x double] undef, double %30, 0
; CHECK-NEXT:   %32 = extractelement <3 x double> %29, i64 1
; CHECK-NEXT:   %33 = insertvalue [3 x double] %31, double %32, 1
; CHECK-NEXT:   %34 = extractelement <3 x double> %29, i64 2
; CHECK-NEXT:   %35 = insertvalue [3 x double] %33, double %34, 2
...
; CHECK-NEXT:   store double 0.000000e+00, double* %"arrayidx'ipg3", align 8
; CHECK-NEXT:   store double 0.000000e+00, double* %"arrayidx'ipg4", align 8
...
; CHECK-NEXT:   %"sum.1.lcssa'" = phi {{(fast )?}}[3 x double] [ %"sum.036'", %for.cond1.preheader ], [ %35, %for.body4 ]
