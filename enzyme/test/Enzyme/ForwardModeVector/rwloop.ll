...
define void @main(double* %a, %struct.Gradients* %da, i32* %N) {
entry:
  %call = call %struct.Gradients (i8*, ...) @__enzyme_fwdvectordiff(i8* bitcast (double (double*, i32*)* @alldiv to i8*), double* nonnull %a, %struct.Gradients* nonnull %da, i32* nonnull %N)
  ret void
}
...
; CHECK: define internal <3 x double> @fwdvectordiffealldiv(double* noalias nocapture %a, <3 x double>* nocapture %"a'", i32* noalias nocapture %N)
; CHECK-NEXT: entry:
...
; CHECK-NEXT:   %"sum.134'" = phi fast <3 x double> [ %"sum.036'", %for.body4.lr.ph ], [ %9, %for.body4 ]
; CHECK-NEXT:   %iv.next2 = add nuw nsw i64 %iv1, 1
; CHECK-NEXT:   %4 = add nuw nsw i64 %iv1, %1
; CHECK-NEXT:   %"arrayidx'ipg" = getelementptr inbounds <3 x double>, <3 x double>* %"a'", i64 %4
; CHECK-NEXT:   %arrayidx = getelementptr inbounds double, double* %a, i64 %4
; CHECK-NEXT:   %5 = load double, double* %arrayidx, align 8, !tbaa !6
; CHECK-NEXT:   %6 = load <3 x double>, <3 x double>* %"arrayidx'ipg", align 8
; CHECK-NEXT:   %.splatinsert = insertelement <3 x double> poison, double %5, i32 0
; CHECK-NEXT:   %.splat = shufflevector <3 x double> %.splatinsert, <3 x double> poison, <3 x i32> zeroinitializer
; CHECK-NEXT:   %7 = fmul fast <3 x double> %6, %.splat
; CHECK-NEXT:   %8 = fadd fast <3 x double> %7, %7
; CHECK-NEXT:   %9 = fadd fast <3 x double> %"sum.134'", %8
...
; CHECK-NEXT:   %"sum.1.lcssa'" = phi fast <3 x double> [ %"sum.036'", %for.cond1.preheader ], [ %9, %for.body4 ]
