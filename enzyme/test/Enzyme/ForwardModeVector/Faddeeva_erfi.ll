...
declare %struct.Gradients @__enzyme_fwdvectordiff({ double, double } ({ double, double })*, ...)
...
entry:
  %0 = tail call %struct.Gradients ({ double, double } ({ double, double })*, ...) @__enzyme_fwdvectordiff({ double, double } ({ double, double })* nonnull @tester, { double, double } %x, %struct.Gradients { { double, double } { double 1.0, double 1.0 }, { double, double } { double 1.0, double 1.0 }, { double, double } { double 1.0, double 1.0 } })
  ret %struct.Gradients %0
}


; CHECK: define internal [3 x { double, double }] @fwdvectordiffetester({ double, double } %in, [3 x { double, double }] %"in'") {
; CHECK-NEXT: entry:
