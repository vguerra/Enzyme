...
define i32 @main() {
entry:
  %call3.4 = tail call %struct.Gradients (double (double)*, ...) @__enzyme_fwdvectordiff(double (double)* nonnull @fun2, double 2.0, [2 x double] [double 0.000000e+00, double 1.000000e+00])
  ret i32 0
}

; CHECK: define internal <2 x double> @fwdvectordiffefun2(double %x, <2 x double> %"x'")
; CHECK-NEXT: entry:
