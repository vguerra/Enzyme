...
define dso_local %struct.Gradients @dsumsquare(double %x) local_unnamed_addr #3 {
entry:
  %0 = tail call %struct.Gradients (double (double)*, ...) @__enzyme_fwdvectordiff(double (double)* nonnull @foo, double %x, [3 x double] [double 1.0, double 2.0, double 3.0])
...
; CHECK: define internal {{(dso_local )?}}<3 x double> @fwdvectordiffefoo(double %x, <3 x double> %"x'")
; CHECK-NEXT: entry:
; CHECK-NEXT:   %0 = call fast <3 x double> @fwdvectordiffesub(double %x, <3 x double> %"x'")
; CHECK-NEXT:   %call1 = tail call fast double @read2()
; CHECK-NEXT:   ret <3 x double> %0
; CHECK-NEXT: }

; CHECK: define internal {{(dso_local )?}}<3 x double> @fwdvectordiffesub(double %x, <3 x double> %"x'")
; CHECK-NEXT: entry:
